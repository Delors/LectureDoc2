...
function initializeStory(story) {
    ...
    story.style.height = parentHeight - offsetTop - parentPaddingBottom + "px";

    function restoreScrollState() {
        scrollStack[story] = [];
        story
            .querySelectorAll(".incremental[style*='visible']")
            .forEach((e) => {
...
function afterLDListenerRegistrations() {
    ...
    document
        .querySelectorAll("#ld-slides-pane ld-story .incremental")
        .forEach((i) => {
            const visibilityChangeObserver = new MutationObserver(
                (mutations) => {
                    for (const mutation of mutations) {
                        ...
                        if (mutation.oldValue.includes("visible")) {
                            ...
                            if (oldScrollState) {
                                // defensive programming if something goes wrong...
            ...
            visibilityChangeObserver.observe(i, {
                attributes: true,
