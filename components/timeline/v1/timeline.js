	...
	return {
		...
		draw: function(svgElement, timeline) {
			...
			for (var i = 0; i < timeline.length; i++) {
				...
				textNodeMaxHeight = Math.max(textNodeMaxHeight, parseInt(window.getComputedStyle(textNode).getPropertyValue("line-height").replace("px", "")));
				...
				dateNodeMaxHeight = Math.max(dateNodeMaxHeight, parseInt(window.getComputedStyle(dateNode).getPropertyValue("line-height").replace("px", "")));

			};
			var widthPerEvent = Math.max(dateNodeMaxWidth + dateNodeMaxHeight, textNodeMaxHeight * r)*0.85;
			var dimensionOfText = dimensionAfterRotation(textNodeMaxWidth, textNodeMaxHeight);
			...
			for (var i = 0; i < timeline.length; i++) {
				...
				var dateNodeX = i * widthPerEvent + ((widthPerEvent - dateNode.getComputedTextLength()) / 2);
